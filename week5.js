...
const phone = data.map((users) => {
  return users.phone;
});
console.log(phone);

const info = data.map((users) => (`${users.website} ${users.email}` ));
console.log(info);
...
const user9toDo = data1.filter((toDo) => toDo.userId === 9);
console.log(user9toDo);

const user9incomplete = user9toDo.filter(task =>(task.completed === false ));
...
const user5post = data2.filter((post) => post.userId ===5);
console.log(user5post);

const titleBody = data2.map((user) => ( `${user.title} ${user.email}` ));
